...
void main() {
	vec4 sampleDataM9 = texture2D(u_sampler1, gl_PointCoord + vec2(-9.0 / 100.0), 0.0);
	vec4 sampleDataM8 = texture2D(u_sampler1, gl_PointCoord + vec2(-8.0 / 100.0), 0.0);
	vec4 sampleDataM7 = texture2D(u_sampler1, gl_PointCoord + vec2(-7.0 / 100.0), 0.0);
	vec4 sampleDataM6 = texture2D(u_sampler1, gl_PointCoord + vec2(-6.0 / 100.0), 0.0);
	vec4 sampleDataM5 = texture2D(u_sampler1, gl_PointCoord + vec2(-5.0 / 100.0), 0.0);
	vec4 sampleDataM4 = texture2D(u_sampler1, gl_PointCoord + vec2(-4.0 / 100.0), 0.0);
	vec4 sampleDataM3 = texture2D(u_sampler1, gl_PointCoord + vec2(-3.0 / 100.0), 0.0);
	vec4 sampleDataM2 = texture2D(u_sampler1, gl_PointCoord + vec2(-2.0 / 100.0), 0.0);
	vec4 sampleDataM1 = texture2D(u_sampler1, gl_PointCoord + vec2(-1.0 / 100.0), 0.0);
	vec4 sampleData = texture2D(u_sampler1, gl_PointCoord);
	vec4 sampleDataP1 = texture2D(u_sampler1, gl_PointCoord + vec2(1.0 / 100.0), 0.0);
	vec4 sampleDataP2 = texture2D(u_sampler1, gl_PointCoord + vec2(2.0 / 100.0), 0.0);
	vec4 sampleDataP3 = texture2D(u_sampler1, gl_PointCoord + vec2(3.0 / 100.0), 0.0);
	vec4 sampleDataP4 = texture2D(u_sampler1, gl_PointCoord + vec2(4.0 / 100.0), 0.0);
	vec4 sampleDataP5 = texture2D(u_sampler1, gl_PointCoord + vec2(5.0 / 100.0), 0.0);
	vec4 sampleDataP6 = texture2D(u_sampler1, gl_PointCoord + vec2(6.0 / 100.0), 0.0);
	vec4 sampleDataP7 = texture2D(u_sampler1, gl_PointCoord + vec2(7.0 / 100.0), 0.0);
	vec4 sampleDataP8 = texture2D(u_sampler1, gl_PointCoord + vec2(8.0 / 100.0), 0.0);
	vec4 sampleDataP9 = texture2D(u_sampler1, gl_PointCoord + vec2(9.0 / 100.0), 0.0);

	vec2 lookup = vec2(10000.0, 0.0); //distance, uv.y adjust
	lookup = testLookup(lookup, v_distort, sampleDataM9, sampleDataM9, -9.0 / 100.0, -8.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM8, sampleDataM7, -8.0 / 100.0, -7.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM7, sampleDataM6, -7.0 / 100.0, -6.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM6, sampleDataM5, -6.0 / 100.0, -5.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM5, sampleDataM4, -5.0 / 100.0, -4.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM4, sampleDataM3, -4.0 / 100.0, -3.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM3, sampleDataM2, -3.0 / 100.0, -2.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM2, sampleDataM1, -2.0 / 100.0, -1.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataM1, sampleData, -1.0 / 100.0, 0.0);
	lookup = testLookup(lookup, v_distort, sampleData, sampleDataP1, 0.0, 1.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP1, sampleDataP2, 1.0 / 100.0, 2.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP2, sampleDataP3, 2.0 / 100.0, 3.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP3, sampleDataP4, 3.0 / 100.0, 4.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP4, sampleDataP5, 4.0 / 100.0, 5.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP5, sampleDataP6, 5.0 / 100.0, 6.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP6, sampleDataP7, 6.0 / 100.0, 7.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP7, sampleDataP8, 7.0 / 100.0, 8.0 / 100.0);
	lookup = testLookup(lookup, v_distort, sampleDataP8, sampleDataP9, 8.0 / 100.0, 9.0 / 100.0);
