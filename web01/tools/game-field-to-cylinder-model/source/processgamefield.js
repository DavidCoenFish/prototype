...
const appendNodeToModel = function(inout_model, in_node){
	var r1 = in_node.cylinder[6];
	var r2 = in_node.cylinder[7];
	var sphereRadius = Math.sqrt((r1 * r1) + (r2 * r2));
	var sphere = [in_node.cylinder[0], in_node.cylinder[1], in_node.cylinder[2], sphereRadius];
	var cylinder = [in_node.cylinder[3], in_node.cylinder[4], in_node.cylinder[5], r1];
	appendArray(inout_model.modelSphereData, sphere);
	appendArray(inout_model.modelCylinderData, cylinder);
...
export default function(in_webGLState){
	return ModelWrapperFactory(
		in_webGLState, "POINTS", ${elementCount}, {
			...
			"a_cylinder" : ModelDataStream(in_webGLState, "FLOAT", 4, Base64ToFloat32Array("${Base64.Float32ArrayToBase64(in_model.modelCylinderData)}"), "STATIC_DRAW", false),
...
const runObjectIDModel = function(in_gameField){
	...
	for (var index = 0; index < in_gameField.nodearray.length; ++index){
		...
		if (!("cylinder" in node) || !("objectid" in node)){
			continue;
		}
