...
const Base64ToFloat32Array = function (b64) {
	...
	for (var index = 0; index < count; ++index){
		result[index] = view.getFloat32(index * 4);
	}
	return result;
}
...
const Float32ArrayToBase64 = function(float32) {
	...
	var view = new DataView(buf);
	float32.forEach(function (b, i) {
		view.setFloat32(i * 4, b);
	});

	var uint8 = new Uint8Array(uint8Count);
	for (var index = 0; index < uint8Count; ++index){
		uint8[index] = view.getUint8(index);
	}
	const result = Uint8ArrayToBase64 (uint8);
	return result;
}
