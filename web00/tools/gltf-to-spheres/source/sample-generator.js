...
const calculateY = function(in_min, in_xIndex, in_yIndex, in_zIndex, in_sphereDiameter){
	const zEven = (0 === (in_zIndex & 1));
	if (zEven){
...
const calculateZ = function(in_min, in_xIndex, in_yIndex, in_zIndex, in_sphereDiameter){
	...
	for (var zIndex = 0; zIndex <= zCount; ++zIndex){
		...
		for (var yIndex = 0; yIndex <= yCount; ++yIndex){
			for (var xIndex = 0; xIndex <= xCount; ++xIndex){
				...
				const localY = calculateY(in_min, xIndex, yIndex, zIndex, in_sphereDiameter);
				const localZ = calculateZ(in_min, xIndex, yIndex, zIndex, in_sphereDiameter);
				if (true === in_spaceInvestigator.isPointInside([localX, localY, localZ])){
