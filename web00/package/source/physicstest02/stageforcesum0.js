...
//get force sum (velocity, gravity, collision)
//	input sphere pos -2
//	input sphere pos -1
...
void main() {
	...
	float radius = positionPrev.w / 2.0;
	float predectedHeight = positionPrev.z + (forceSum.z * u_timeStep * u_timeStep);

	float height = predectedHeight - radius;
	float zPlaneHeight = 0.0;
	if (height < zPlaneHeight){
		float targetDisplacment = radius - positionPrev.z + zPlaneHeight;
...
const factory = function(in_resourceManager, in_webGLContextWrapper, in_webGLState, in_dataServer){
	...
	const m_renderTargetData = WebGL.RenderTargetData.factory(in_dataServer.getTextureForceSum0(), "FRAMEBUFFER", "COLOR_ATTACHMENT0", "TEXTURE_2D");
	const m_renderTarget = WebGL.RenderTargetWrapper.factory(in_webGLContextWrapper, in_dataServer.getTextureForceSum0().getWidth(), in_dataServer.getTextureForceSum0().getHeight(), [m_renderTargetData]);
