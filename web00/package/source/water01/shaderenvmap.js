...
vec3 calcViewNorm(vec2 in_viewXY){
	float distSquared = dot(in_viewXY, in_viewXY);
	if (1.0 < distSquared){
		float dist = sqrt(distSquared);
		vec3 horzonNorm = vec3(in_viewXY.x / dist, in_viewXY.y / dist, 0.0);
		return horzonNorm;
	}
	float viewZ = sqrt(1.0 - distSquared);
	vec3 viewNorm = vec3(in_viewXY.x, in_viewXY.y, viewZ);
	return viewNorm;
}

void main() {
	vec3 viewNorm = calcViewNorm(v_position);
